...
public class AppointmentService {
    ...
    public Page<AppointmentResponse> findAppointmentByDoctorIdAndDateRange(Long doctorId, LocalDateTime appointmentDay, int pageNumber, int pageSize) {

        Pageable pageable = PageRequest.of(pageNumber, pageSize);

        if (doctorId == null) {
            return appointmentRepository.findByAppointmentDate(appointmentDay, pageable).map(appointment -> modelMapper.map(appointment, AppointmentResponse.class));
        }
        return appointmentRepository.findByDoctorIdAndAppointmentDate(doctorId, appointmentDay, pageable).map(appointment -> modelMapper.map(appointment, AppointmentResponse.class));
    }

    public Page<AppointmentResponse> findAppointmentByAnimalIdAndDateRange(Long animalId, LocalDateTime appointmentDay, int pageNumber, int pageSize) {
        ...
        return appointmentRepository.findByAnimalIdAndAppointmentDateBetween(animalId, appointmentDay, pageable).map(appointment -> modelMapper.map(appointment, AppointmentResponse.class));
    }
